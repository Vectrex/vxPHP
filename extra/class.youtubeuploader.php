...
class YoutubeUploader {
	...
	public function __construct($credentialsFile) {
		if(file_exists($credentialsFile) && $xml = simplexml_load_file($credentialsFile)) {
			...
			throw new YoutubeUploaderException("Credentials file '$credentialsFile' not found.");
		}
	}
	...
	public function getAuth() {
		...
		if($status < 200 || $status >= 300) {
			throw new YoutubeUploaderException('getAuth() failed!');
		}		
	...
	public function uploadFile($path, $name = NULL) {
		if(!file_exists($path)) {
			throw new YoutubeUploaderException("uploadFile() failed! File '$path' not found.");
		}
	...
	public function setTitle($title) {
		...
		if(trim($title) == '') {
			throw new YoutubeUploaderException('setTitle() failed! No valid title specified.');
		}
	...
	public function setDescription($description) {
		...
		if(trim($description) == '') {
			throw new YoutubeUploaderException('setDescription() failed! No valid description specified.');
		}
	...
	public function setKeywords($keywords) {
		...
		if(empty($filtered)) {
			throw new YoutubeUploaderException('setKeywords() failed! No valid keywords specified.');
		}
	...
	public function getAssignableCategories() {
		if(!isset($this->assignableCategories)) {
			...
			if($xml === FALSE) {
				throw new YoutubeUploaderException('getAllowedCategories() failed! Category file not found or not valid XML.');
			}
...
class YoutubeUploaderException extends Exception {
