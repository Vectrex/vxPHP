...
*/	}
	...
	function printHTML($size = "2px") {
		...
		for ($r = 0; $r < $this->getModuleCount(); $r++) {
			...
			for ($c = 0; $c < $this->getModuleCount(); $c++) {
				$color =$this->modules[$r][$c] ? "#000000" : "#ffffff";
...
class QRUtil {
	...
	function getLostPoint($qrCode) {
		...
		for ($col = 0; $col < $moduleCount; $col++) {
			for ($row = 0; $row < $moduleCount; $row++) {
				if (/*$qrCode->isDark($row, $col)*/$qrCode->modules[$row][$col] ) {
					$darkCount++;
				}
			}
		}
...
//		var_dump($QR_MATH_LOG_TABLE);
//		var_dump($QR_MATH_EXP_TABLE);
	}
