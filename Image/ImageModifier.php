...
class ImageModifier {
	...
	private function do_resize() {
		...
		if(count($args) >= 2) {
			...
			// no limit
				...
				if($width != 0 || $height != 0) {
					...
					throw new ImageModifierException("Invalid dimension(s) for do_resize(): $width, $height");
				}
			}
		}
		...
		// single float value given
			...
			if(!is_numeric($args[0]) || $args[0] == 0) {
				throw new ImageModifierException("Invalid dimension(s) for do_resize(): {$args[0]}");
			}
			$width	= round($src->width * $args[0]);
			$height	= round($src->height * $args[0]);
		}
		else {
			throw new ImageModifierException('Insufficient arguments for do_resize()');
		}
