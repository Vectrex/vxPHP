...
class sendMail {
	...
	public function __construct($receiver = null, $subject = '(Kein Betreff)', $mailText = '', $sender = DEFAULT_MAIL_SENDER, $bcc = '', $sig = '', $htmlMail = false) {
		...
		$this->encoding	= defined('DEFAULT_ENCODING') ? strtoupper(DEFAULT_ENCODING) : 'ISO-8859-1'; 
	}
	...
	public function send()	{
		...
		if (is_array($this->receiver)) {
			foreach ($this->receiver as $r) {
				...
				if($this->debug) {
					echo '<div style="border: solid 2px #888; background:#efe; font-family: monospace; font-size: 1em; padding: 1em; margin: 1em;">';
					echo $this->receiver.'<hr>'.nl2br($this->headers).'<hr>'.nl2br($this->msg);
					echo '</div>';
				}
				else {
