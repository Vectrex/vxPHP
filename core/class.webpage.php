...
abstract class Webpage {
	...
	public function __construct() {
		if(!isset($GLOBALS['config'])) {
			throw new Exception('Configuraton object not found!');
		}
		if(isset($GLOBALS['db'])) {
			$this->db = &$GLOBALS['db'];
		}
	...
	private function setMetaValue($name) {
		...
		if(!isset($metaData) && method_exists('TemplateAdmin', 'getPageMetaData')) {
			$metaData = TemplateAdmin::getPageMetaData($this->currentPage);
			if(!empty($metaData)) {
